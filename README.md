...
> **This repo replaced `spandaapp/sadhanatools-sadhana-datapacks` on 2026-08-11.** It starts
> from a single commit; the old history is not carried over. Content was migrated
> byte-for-byte.
>
> **Do not delete the old repo.** `GITHUB_DATAPACKS_REPO` is a compile-time `BuildConfig`
> field, so every already-installed copy of the app still reads the old repo and will keep
> doing so until its user updates. The old repo is frozen: it stays up, serving the catalog
> those installs expect. New pack submissions only land here, so the two diverge from now on
> — that is expected, and it resolves as users update.
>
> Existing bug-report issues also live on the old repo, and their screenshots are committed
> to its `feedback-screenshots` branch. Deleting it breaks the image in every one of them.
...
```powershell
.\datapacks-repo\push-datapacks-repo.ps1 -Owner spandaapp -Repo sadhana-packs
```

Legacy export from APK assets is no longer used.

Create a public repo (e.g. `spandaapp/sadhana-packs`), then from the tool repo root:
...
GITHUB_DATAPACKS_REPO=sadhana-packs
```

Fine-grained PAT scoped to `sadhana-packs` only — no admin/bypass rights:
...
3. Create a GitHub App for automerge (Settings → Developer settings → GitHub Apps → New
   ...
   *Migrating from an existing datapacks repo?* Don't create a second App — open the one you
   already have (`spanda-datapacks-automerge`), add this repo under **Install App →
   Configure → Repository access**, and reuse its existing App ID and private key for the
   secrets below. One App installed on both repos is the intended shape; a new App means a
   new ID, a new key, and a second bypass actor to keep straight.
